...
RUN git clone https://github.com/thu-ml/SageAttention.git /build/SageAttention \
    ...
    && TORCH_CUDA_ARCH_LIST="7.5 8.0 8.6 8.7 8.9" python3.12 -m pip wheel --no-build-isolation . -w /wheels --verbose --config-settings="cmake.args=-DCMAKE_CUDA_ARCHITECTURES=75;80;86;87;89" \
    && cd /build \
    && rm -rf SageAttention
