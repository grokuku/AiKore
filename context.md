...
---
    ...
    *   **Bug d'Affichage VNC ("View") :** Dans la vue embarquée ("View"), l'interface VNC ne s'adapte pas dynamiquement à la taille du panneau, créant une petite zone d'affichage fixe dans un grand cadre.
    *   **Bug du Bouton "View" (obsolète) :** Le bouton "View" pour les instances persistantes ne fonctionne pas correctement et renvoie une erreur "Not Found" dans la vue embarquée.
    *   **Fonctionnalité de Mise à Jour Non Implémentée :** Le bouton "Update" sur chaque ligne d'instance est actuellement un placeholder.
    
    ### 6.3. Journal d'Investigation
    
    *   **2025-11-06 :**
        *   **Objectif :** Corriger le bug de la résolution VNC fixe pour les instances persistantes.
        *   **Problèmes Résolus :**
            1.  **Redimensionnement du conteneur `iframe` :** Modification de `style.css` pour que le conteneur de la vue embarquée utilise flexbox, lui permettant de remplir correctement tout l'espace disponible dans le panneau des outils.
            2.  **Redimensionnement du bureau VNC (Mode "Open") :** Après une longue investigation (recherche d'un script `autoresize` manquant, test de l'option `-ClientEventCmd` non supportée), la cause racine a été identifiée comme un problème de timing au démarrage. Le gestionnaire de fenêtres `openbox` tentait de se lancer avant que le serveur `Xvnc` n'ait initialisé l'affichage.
            *   **Solution :** Le script `scripts/kasm_launcher.sh` a été réécrit pour lancer `Xvnc` en arrière-plan, attendre 2 secondes, puis lancer `openbox` et le blueprint. Cela garantit un démarrage dans le bon ordre et fiabilise le redimensionnement natif de KasmVNC (via l'option `AcceptSetDesktopSize`).
        *   **État à la fin de la session :** Le redimensionnement du bureau VNC fonctionne maintenant parfaitement lorsque l'instance est ouverte dans un nouvel onglet (bouton "Open"). Un bug subsiste pour la vue embarquée (bouton "View").
    
    *   **2025-11-05 :**
        *   **Objectif :** Migration vers une nouvelle image de base `linuxserver/baseimage-kasmvnc:ubuntunoble`.
        *   **Problèmes Résolus :** Corrections multiples de build Docker, de permissions au démarrage, de conflits de services s6, et fiabilisation du script de lancement KasmVNC et du monitoring.
        *   **État à la fin de la session :** Le conteneur est stable et les instances persistantes se lancent avec succès.
    
    ### 6.4. Plan d'Action pour la Prochaine Session
    
    *   **Priorité 1 : Corriger le bug de redimensionnement VNC dans la vue embarquée ("View").**
        *   **Hypothèse :** Le serveur VNC est maintenant correctement configuré. Le problème est probablement côté client : le client JavaScript KasmVNC, qui tourne à l'intérieur de l'iframe, n'est pas informé lorsque la taille de l'iframe elle-même change (par exemple, lors du redimensionnement des panneaux de l'UI).
        *   **Action :** Investiguer une solution en JavaScript dans `app.js`. L'approche la plus probable consistera à utiliser un `ResizeObserver` sur le conteneur de l'iframe (`#instance-view-container`) pour détecter les changements de taille et appeler une fonction du client KasmVNC pour qu'il se synchronise avec le serveur.
    
    *   **Priorité 2 : Corriger le bouton "View" pour les instances non-persistantes.**
        *   **Action :** La logique actuelle dans `app.js` pour les instances non-persistantes est correcte, mais le reverse-proxy NGINX n'est peut-être pas configuré pour gérer le `slug` du nom de l'instance. Vérifier la configuration NGINX et la logique de `slugify`.