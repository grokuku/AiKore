    ...
    *   **Responsibilities:**
    ...
    4.  **Phase 4: Advanced Features & UX (In Progress)**
        *   **Completed in this Phase:**
            *   Architected and implemented a "Persistent Session (VNC)" mode that launches a fully isolated, per-instance VNC server and web client, ensuring session persistence and complete isolation between instances.
            *   Updated the database model, API schemas, and CRUD operations to support and track the state of these isolated VNC sessions.
        *   **TODO:**
            *   Resolve the "502 Bad Gateway" error, which indicates the VNC web client process (`websockify`) is failing to start correctly within the dynamically generated launch script.
