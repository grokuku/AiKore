...
def update_comfyui_proxy(db: Session):
    ...
    if active_instance and active_instance.status == "started":
        print(f"[Manager] Activating ComfyUI proxy for '{active_instance.name}' on port {active_instance.port}")
        ...
        upstream_content = textwrap.dedent(f"""\
            ...
            upstream comfyui_active_slot {{
                server 127.0.0.1:{active_instance.port};
            }}
        """)
            ...
            if elapsed_time > STALLED_TIMEOUT:
                with SessionLocal() as db:
                    ...
                    if instance and instance.status == "starting":
                        ...
                        db.commit()
                        print(f"[Monitor-{instance_id}] Instance has been starting for >{STALLING_TIMEOUT}s. Marked as STALLED.")
...
def start_instance_process(db: Session, instance: models.Instance):
    ...
    # MODIFIED: Ensure both NGINX config directories exist
    ...
    # NGINX Configuration for persistent mode is handled here.
    # Standard mode proxying is now handled by update_comfyui_proxy.
    if instance.persistent_mode:
...
def stop_instance_process(db: Session, instance: models.Instance):
    ...
    if instance.persistent_mode:
        # Corrected path for persistent mode cleanup
        _cleanup_instance_files(_slugify(instance.name))
    
    # ADDED: Ensure ComfyUI proxy is removed if the stopped instance was the active slot.
    # This is crucial to prevent the proxy from pointing to a dead port.
    metadata = parse_blueprint_metadata(instance.base_blueprint)
    if metadata.get('app_id') == 'comfyui' and not instance.persistent_mode and instance.is_comfyui_active_slot:
        # We call the update function here. It will see that no active instance is "started"
        # and will proceed to remove the configuration file automatically.
        print(f"[Manager] Instance '{instance.name}' was the active ComfyUI slot. Triggering proxy deactivation.")
        # The logic inside update_comfyui_proxy is now robust enough to handle this.
        update_comfyui_proxy(db)
