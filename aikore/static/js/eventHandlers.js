...
import { createInstance, performInstanceAction, cloneInstance, instantiateInstance, updateInstanceScript } from './api.js';
import { showToolsMenu, hideToolsMenu, hideAllModals } from './modals.js';
import { openEditor, openTerminal, showVersionCheckView, openInstanceView, showLogViewer } from './tools.js';
import { getBestGpu } from './api.js';
import { renderInstanceRow, buildInstanceUrl } from './ui.js';
...
export function setupMainEventListeners() {
    DOM.addInstanceBtn.addEventListener('click', async () => {
        ...
        const bestGpuId = await getBestGpu();
    ...
    DOM.instancesTbody.addEventListener('click', async (event) => {
        ...
        switch (action) {
            ...
            case 'stop':
                ...
                await performInstanceAction(instanceId, action);
                break;
    ...
    DOM.toolsContextMenu.addEventListener('click', (event) => {
        const target = event.target.closest('[data-action]');
    ...
    DOM.editorUpdateBtn.addEventListener('click', () => {
        const row = DOM.instancesTbody.querySelector(`tr[data-id="${state.editorState.instanceId}"]`);
        if (!row) {
